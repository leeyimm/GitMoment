...
class GTMAPIManager {
    ...
    func fetchRepoPullRequests(ownername: String, reponame: String, page: Int, completionHandler: @escaping (Result<([GTMPullRequest], Int)>) ->Void ) {
        Alamofire.request(GTMAPIRouter.getRepoPullRequests(ownername, reponame)).responseJSON { (response) in
            guard response.result.error == nil else {
                print(response.result.error!)
                completionHandler(.failure(GTMAPIManagerError.network(error: response.result.error!)))
                return
            ...
            let pullRequests : [GTMPullRequest] = Array(JSONArray: jsonArray)
            completionHandler(.success((pullRequests, page)))
        }
    }
    ...
    func fetchPullRequestFiles(ownername: String, reponame: String, pullNum: Int, page: Int, completionHandler: @escaping (Result<([GTMUpdatedFileInfo], Int)>) ->Void ) {
        Alamofire.request(GTMAPIRouter.getPullRequestFiles(ownername, reponame, pullNum)).responseJSON { (response) in
            guard response.result.error == nil else {
                print(response.result.error!)
                completionHandler(.failure(GTMAPIManagerError.network(error: response.result.error!)))
                return
            ...
            let files : [GTMUpdatedFileInfo] = Array(JSONArray: jsonArray)
            completionHandler(.success((files, page)))
        }
    }
    ...
    func fetchREADMEContent(ownername: String, reponame: String, completionHandler: @escaping(Result<String>) -> Void) {
        Alamofire.request(GTMAPIRouter.getREADMEContent(ownername, reponame)).responseString { (response) in
            guard response.result.error == nil else {
                print(response.result.error!)
                completionHandler(.failure(GTMAPIManagerError.network(error: response.result.error!)))
                return
